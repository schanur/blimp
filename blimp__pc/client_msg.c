...
static int setup_client (key_t key, int flag) {
   ...
   res = msgget (key, flag);
   if (res == -1) {
      printf ("Clientfehler beim Einrichten"
              " der Message Queues...\n");
      return -1;
   }
   return res;
...
int main (void) {
   ...
   while (ende) {
  /* Hier könnte der wichtige Code zur Kommunikation zwischen  */
  /* den Prozessen geschrieben werden. In diesem Beispiel      */
  /* werden nur die neu erstellten Message Qeues als neue User */
  /* ausgegeben.                                               */
  /* Die Schleife wartet auf das Signal SIGINT == STRG+C       */

      /* Bestätigung oder Rundschreiben empfangen */
      res = msgrcv (client_id, &s2c, MSG_LEN, 0, IPC_NOWAIT);
	if (res != -1) {
         //printf ("(%s) von User mit der Message Queues-ID: "
           //      " %ld\n", s2c.message, s2c.prioritaet);
      		if (s2c.prioritaet == client_id) {
			printf ("(%s) nachricht Queues-ID: %ld\n", s2c.message, s2c.prioritaet);
		} else {
			printf ("(%s) falsche: %ld\n", s2c.message, s2c.prioritaet);
		}
	}
